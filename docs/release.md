...
`@autor:reef425`
`@note_start:`
>1. Добавил модуль `ui/GamePages/suwidgets/GameVariantAnimation.py`, класс `GameVariantAnimation` наследуется от `QVariantAnimation`,
содержит в себе стандартные задержки анимации для разных типов объектов.
>2. Доработал запуск игры.
 * Сначала создаются характеристики героя.
 * Загружается страница для обноления характеристик  героя.
 * Загружается уроень и нинциализируются данные уровня.
 * Создаются страницы игры, создаются графические элементы.
>3. Переработал конфигурироание игры, добаил отдельные модули в пакет `gameconfig`
```
ui/gameconfig
├── __init__.py
├── DeviceConfig.py
...
└── UserConfig.py
```
>4. Добавил анимацию для вылетающей цифры урона.
>5. Добавил анимацию движения юнита.
>6. Добавил анимацию исчезноение текста.
>7. Исправлено настройки разршения экрана.
