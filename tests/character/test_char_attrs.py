...
def test_reset(char, some_attribute):
    char.unit.xp = int(1e10)

    before = get_attrib_by_enum(char.unit, some_attribute)

    char.increase_attrib(some_attribute)

    assert get_attrib_by_enum(char.unit, some_attribute) > before
