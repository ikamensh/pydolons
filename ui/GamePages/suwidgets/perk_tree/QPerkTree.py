...
from ui.GamePages.suwidgets.perk_tree.QPerkGroup import QPerkGroup
...
class QPerkTree(QWidget):
    def __init__(self, cfg, perk_tree: PerkTree, character: Character, parent=None):
        ...
        self.name = QLabel(perk_tree.name, self)
        layout.addWidget(self.name)
        self.total_spent = QLabel( str(perk_tree.spent_xp), self)
        layout.addWidget(self.total_spent)


        self.visuals = {}
        for perk_group in perk_tree.perk_groups:
            qpg = QPerkGroup(self.cfg, perk_group, self )
